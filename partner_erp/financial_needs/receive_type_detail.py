...
class ReceiveTypeDetail(models.Model):
    """收发类别明细"""
    _name = 'partner.receive_type_detail'
    _description = u"收发类别明细"
    _inherit = ['mail.thread']
    READONLY = {'draft': [('readonly', False)]}

    name = fields.Char(string=u'名称', index=True, readonly=True, states=READONLY)
    receive_type_id = fields.Many2one('partner.receive_type', string=u'收发类别')
    ...
    def unlink(self):
        ...
        if self.state == 'done' or self.state == 'confirm':
            ...
            return super(ReceiveTypeDetail, self).unlink()