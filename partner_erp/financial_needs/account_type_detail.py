...
class AccountTypeDetail(models.Model):
    """账户类别明细"""
    _name = 'partner.account_type_detail'
    _description = u"账户类别明细"
    _inherit = ['mail.thread']
    READONLY = {'draft': [('readonly', False)]}

    name = fields.Char(string=u'名称', index=True, readonly=True, states=READONLY)
    account_type_id = fields.Many2one('partner.account_type', string=u'账户类别', readonly=True, states=READONLY)
    ...
    def unlink(self):
        ...
        if self.state == 'done' or self.state == 'confirm':
            ...
            return super(AccountTypeDetail, self).unlink()