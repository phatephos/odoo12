...
class BudgetItems(models.Model):
    """预算项目"""
    _name = 'partner.budget_items'
    _description = u"预算项目"
    _inherit = ['mail.thread']
    READONLY = {'draft': [('readonly', False)]}

    name = fields.Char(string=u'预算项目', index=True, readonly=True, states=READONLY)
    department_id = fields.Many2one('partner.budget_department', string=u'归口管理部门（预算部门）', readonly=True, states=READONLY)
    ...
    def unlink(self):
        ...
        if self.state == 'done' or self.state == 'confirm':
            ...
            return super(BudgetItems, self).unlink()
