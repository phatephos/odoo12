...
class ExpendDetail(models.Model):
    """预算装置"""
    _name = 'partner.expend_detail'
    _description = u"支出明细"
    _inherit = ['mail.thread']
    READONLY = {'draft': [('readonly', False)]}

    name = fields.Char(string=u'支出明细名称', index=True, readonly=True, states=READONLY)
    expend_items_id = fields.Many2one('partner.expend_items', string=u'支出项目', readonly=True, states=READONLY, store=True)
    expenditure_business_id = fields.Many2one('partner.expenditure_business', string=u'支出业务',
                                              related='expend_items_id.expenditure_business_id', readonly=True, states=READONLY, store=True)
    ...
    def unlink(self):
        ...
        if self.state == 'done' or self.state == 'confirm':
            ...
            return super(ExpendDetail, self).unlink()
