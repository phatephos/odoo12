...
class BillType(models.Model):
    """单据类型"""
    _name = 'partner.bill_type'
    _description = u"单据类型"
    _inherit = ['mail.thread']
    READONLY = {'draft': [('readonly', False)]}

    name = fields.Char(string=u'单据类型名称', index=True, readonly=True, states=READONLY)
    ir_model_id = fields.Many2one('ir.model', string='查询单据', index=True, required=True, readonly=True, states=READONLY)
    ...
    def unlink(self):
        ...
        if self.state == 'done' or self.state == 'confirm':
            ...
            return super(BillType, self).unlink()
