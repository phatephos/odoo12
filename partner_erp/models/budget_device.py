...
class BudgetDevice(models.Model):
    """预算装置"""
    _name = 'partner.budget_device'
    _description = u"预算装置"
    _inherit = ['mail.thread']
    READONLY = {'draft': [('readonly', False)]}

    name = fields.Char(string=u'预算装置名称', index=True, readonly=True, states=READONLY)
    company_id = fields.Many2one('partner.archives', domain="[('is_company', '=', True)]", string=u'公司', readonly=True, states=READONLY)
    production_system_id = fields.Many2one('partner.production_system', string=u'生产加工系统', readonly=True, states=READONLY)
    ...
    def unlink(self):
        ...
        if self.state == 'done' or self.state == 'confirm':
            ...
            return super(BudgetDevice, self).unlink()
